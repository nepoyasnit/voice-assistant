...
# def message_partition(message_text):
#     if len(message_text) > 250:
#         index = 249
#         while index > 0:
#             if message_text[index] == '.' or message_text[index] == '!' or message_text[index] == ':' or message_text[
#                 index] == ',' or message_text[index] == '?' or message_text[index] == ' ':
#                 result = message_text[:index] + message_partition(message_text[index:])
#
#     return [].append(message_text)
    ...
    def get(self, request):  # Decide if to create dialog or upload previous messages
        if request.user.is_authenticated:
            try: # the aim of this statement is to create or find dialog anyway
                ...
                messages = Message.objects.filter(dialog__user_id=request.user.id).order_by('date') # this can return an empty list
            return render(request, self.template_name, {'messages': messages}) # returning messanger page with the message history for authenticated user
        return render(request, self.template_name) # if user isn't authenticated, we return a default page with the link to log in

    def post(self, request):  # adds messages to the template
        # the user is already authenticated anyway
        current_dialog = Dialog.objects.get(user_id=request.user.id) # the dialog is guaranted created thanks to the get method
        ...
        Message.objects.create(text=message, dialog=current_dialog, is_mine=True) # create two Message instances related to this dialog
...
class Register(View): # for user's authentication
    ...
    def post(self, request):
        ...
        return render(request, self.template_name, context) # returning back form with invalid info
