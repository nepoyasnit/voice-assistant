...
class VoiceAssistant:
    ...
    def talk(voice):
        ...
        """
        answer = talk_model.predict([voice])[0]
        return answer
...
def setup_assistant_voice():
    ...
    if ttsEngine.assistant.speech_lang == "en":
        ttsEngine.assistant.recognition_lang = "en-US"
        if ttsEngine.assistant.sex == "male":
        ...
        ttsEngine.assistant.recognition_lang = "ru-RU"
...
def search_on_wikipedia(*args: tuple):
    ...
    try:
        if wiki_page.url != "":
            ...
            play_voice_assistant_speech("Can't find " + search_term + "on Wikipedia. /"
...
def check_commands(voice_input, is_work, just_talk, is_talk):
    """
    ...
    if voice_input == "goodbye" or voice_input == "bye":
        ...
        just_talk = True
        play_voice_assistant_speech("I love to chat! If you get tired, say stop talking")
        is_talk = False
    elif voice_input.find("find in wikipedia") != -1:
        search_on_wikipedia(voice_input[18:])
        is_talk = False
    elif voice_input.find("search on youtube") != -1:
        video_search(voice_input[18:])
        is_talk = False
...
def talk():
    ...
    while is_work:
        ...
        if not just_talk:
            is_work, just_talk, is_talk = check_commands(voice_input, is_work, just_talk, is_talk)
        if voice_input == "stop talking":
            just_talk = False
        if is_talk:
            assistant_answer = VoiceAssistant.talk(voice_input)
            play_voice_assistant_speech(assistant_answer)
...
if __name__ == "__main__":
    ...
    ttsEngine.assistant = VoiceAssistant()
    ttsEngine.assistant.name = "Blonde"
    ttsEngine.assistant.sex = "female"
    ttsEngine.assistant.speech_lang = "en"
